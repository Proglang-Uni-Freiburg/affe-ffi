...
(for_f (j0 + 1) (size - 1)
   (fun j ->
    let cell = Intset.rm (get g ( i0, j )) n in set_mut g ( i0, j ) cell))
let remove_column i0 j0 g n =
(for_f (i0 + 1) (size - 1)
   (fun i ->
    let cell = Intset.rm (get g ( i, j0 )) n in set_mut g ( i, j0 ) cell))
    ...
    for_f (3 * pos_j) ((3 * (pos_j + 1)) - 1)
      (fun j ->
       match and_f (i = i0) (j = j0) with
         | false ->
           let cell = Intset.rm (get g ( i, j )) n in set_mut g ( i, j ) cell
         | true -> ())))
let is_valid g = (Intset.for_all (fun x -> (Intset.cardinal x) > 0) g)
let is_solved g = (Intset.for_all (fun x -> (Intset.cardinal x) = 1) g)
let next_pos  =
(fun ( i, j ) ->
 match j < (size - 1) with
   | true -> ( i, j + 1 )
     ...
     | false ->
       ...
       let try_solution =
         ...
         match is_valid new_g with
           ...
           | false -> ()
       in Intset.iter_set try_solution s
 in solve 0 0 g)
let main  = (Intset.print Intset.easy_board;

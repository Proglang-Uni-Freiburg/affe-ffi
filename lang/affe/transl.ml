...
let transl_type_constructor
  ...
  let typ = match constructor.typ with
  | None -> Types.App (typname, typarams)
  | Some t -> (let constructor_typ = transl_type ~ienv t in
              Types.Arrow (constructor_typ, Un Global, App (typname, typarams))) 
  in
...
let transl_decl ~env
  ...
  let typs =
    List.filter_map (fun {T. typ; _} -> match typ with 
                     | None -> None
                     | Some t -> Some (transl_type ~ienv t)) constructor
  in

...
type command =
  ...
  | ValueDef of def
  | Import of string
  | Extern of Name.t * command list
  | OCAML of command * pattern list * string
...
module Rename = struct
  ...
  let rename_constructor
    ...
    | None -> None
    | Some t -> Some (type_expr ~kvarenv ~tyenv ~tyvarenv t) in
    let constraints = constrs ~kvarenv ~tyvarenv ~tyenv constraints in
  ...
  let command { env ; tyenv } = function
    ...
    | ValueDef { name = {name} ; typ } ->
      ...
      ValueDef { name ; typ }
    | Import s -> Import s
    | Extern (n, cmds) -> Extern (n, cmds)
    | OCAML (c, args, s) -> OCAML (c, args, s)
